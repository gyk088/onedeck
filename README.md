...
ONEDECK позволяет легко делить ваше приложени на модули и собирать приложение как конструктор из набора модулей.
В дальнейшем модули легко переиспользовать.

##### Пример приложения [EXAMPLE][example-site] [Git][example-git]
...
Модуль может создовать события c помощью методов `$$on` или `$$onOnce`
Желательно события создавать в методе `eventHandler`, но возможны и другие варианты.
...
Мудуль ROOT реализует паттерн [Mediator][mediator]. Все глобальные события создает модуль ROOT.
События создются точно так-же как и в обчном модуле, помощью методов `$$on` или `$$onOnce`.
```
import ExampleNotification from 'ExampleNotification/module';
import ExampleGlobalWnd from 'ExampleGlobalWnd/module';
